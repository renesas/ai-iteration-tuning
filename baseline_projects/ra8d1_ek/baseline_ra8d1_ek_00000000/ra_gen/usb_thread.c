...
static StaticTask_t usb_thread_memory;
                ...
                static uint8_t usb_thread_stack[4096] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t usb_thread_stack[4096] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.usb_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
void usb_thread_create(void)
{
    ...
    usb_thread = xTaskCreateStatic (
                    ...
                    #endif
                                    usb_thread_func,
                                    (const char*) "USB Thread", 4096 / 4, // In words, not bytes
